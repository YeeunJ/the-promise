...
export function useOccupiedSlots(spaceId: number | null, date: string): UseOccupiedSlotsResult {
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/spaces/${spaceId}/reservations/?date=${encodeURIComponent(date)}`,
